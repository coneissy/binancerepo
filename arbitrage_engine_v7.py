...
# Canonical configured costs. Bid/ask crossing is already represented by gross;
# SLIP is therefore an additional depth/impact allowance only.
FEE_BPS = float(os.getenv("ARB_FEE_BPS", "4"))
SLIP_BPS = float(os.getenv("ARB_SLIPPAGE_BPS", "2"))
FUNDING_BUFFER_BPS = float(os.getenv("ARB_FUNDING_BUFFER_BPS", "1"))
...
def _q(x):
    return pricing.Quote(bid=x[0], ask=x[1], bid_qty=x[2], ask_qty=x[3], ts_ms=x[4])
...
def _tri_diag(a,b,q,eq,venue="spot",engine="SPOT_TRIANGULAR"):
    ...
    qa,qab,qb=_q(pa),_q(pab),_q(pb)
    now=time.monotonic()*1000
    stale=pricing.max_staleness_ms([qa,qab,qb],now)
    if stale > core.STALE:
        _record_rejection(engine,a+b+q,"stale_leg",0.0,0.0,{"staleness_ms":round(stale,3),"stale_limit_ms":core.STALE}); return None
    gross,path_dir=pricing.triangular_gross_bps(qa,qab,qb)
    cost=pricing.cost_bps(3,FEE_BPS,SLIP_BPS, FUNDING_BUFFER_BPS if venue=="fut" else 0.0)
    net=pricing.net_bps(gross,3,FEE_BPS,SLIP_BPS,FUNDING_BUFFER_BPS if venue=="fut" else 0.0)
    path=f"USDT->{a}->{b}->USDT" if path_dir=="FORWARD" else f"USDT->{b}->{a}->USDT"
    if gross<=0:
        _record_rejection(engine,a+b+q,"negative_gross_edge",gross,net,{"path":path,"cost_bps":cost,"staleness_ms":round(stale,3)}); return None
    if net<core.MIN_NET:
        _record_rejection(engine,a+b+q,"below_min_net",gross,net,{"path":path,"min_net_bps":core.MIN_NET,"cost_bps":cost,"staleness_ms":round(stale,3)}); return None
    if net>core.MAX_NET:
        _record_rejection(engine,a+b+q,"above_max_net_safety",gross,net,{"path":path,"max_net_bps":core.MAX_NET,"cost_bps":cost,"staleness_ms":round(stale,3)}); return None
    return {"engine":engine,"type":engine,"path":path,"symbol":a+b+q,"gross_bps":gross,"net_bps":net,"notional_usdt":pricing.target_notional_usdt(eq,core.RISK,core.MAX_NOTIONAL),"paper_only":False,"diagnostic":{"fees_bps":3*FEE_BPS,"slippage_bps":3*SLIP_BPS,"funding_bps":FUNDING_BUFFER_BPS if venue=="fut" else 0,"total_cost_bps":cost,"staleness_ms":round(stale,3),"price_model":"executable_bid_ask"}}
...
def _basis_diag(s,eq):
    ...
    qs,qf=_q(sp),_q(fu)
    stale=pricing.max_staleness_ms([qs,qf],time.monotonic()*1000)
    if stale > core.STALE:
        _record_rejection("SPOT_FUTURES_BASIS",s,"stale_leg",0.0,0.0,{"staleness_ms":round(stale,3),"stale_limit_ms":core.STALE}); return None
    gross,direction=pricing.basis_gross_bps(qs,qf)
    cost=pricing.cost_bps(2,FEE_BPS,SLIP_BPS,FUNDING_BUFFER_BPS)
    net=pricing.net_bps(gross,2,FEE_BPS,SLIP_BPS,FUNDING_BUFFER_BPS)
    if gross<=0:
        _record_rejection("SPOT_FUTURES_BASIS",s,"negative_gross_edge",gross,net,{"direction":direction,"cost_bps":cost,"staleness_ms":round(stale,3)}); return None
    if net<core.MIN_NET:
        _record_rejection("SPOT_FUTURES_BASIS",s,"below_min_net",gross,net,{"direction":direction,"min_net_bps":core.MIN_NET,"cost_bps":cost,"staleness_ms":round(stale,3)}); return None
    if net>core.MAX_NET:
        _record_rejection("SPOT_FUTURES_BASIS",s,"above_max_net_safety",gross,net,{"direction":direction,"cost_bps":cost,"staleness_ms":round(stale,3)}); return None
    return {"engine":"SPOT_FUTURES_BASIS","type":"SPOT_FUTURES_CROSS","symbol":s,"direction":direction,"basis_bps":gross,"gross_bps":gross,"net_bps":net,"notional_usdt":pricing.target_notional_usdt(eq,core.RISK,core.MAX_NOTIONAL),"paper_only":True,"diagnostic":{"fees_bps":2*FEE_BPS,"slippage_bps":2*SLIP_BPS,"funding_bps":FUNDING_BUFFER_BPS,"total_cost_bps":cost,"staleness_ms":round(stale,3),"price_model":"executable_bid_ask"}}
...
def stats_v7():
    out=_original_stats(); out["engine"]="dragon-v7"; out["live_execution"]=enabled(); out["live_mode_configured"]=_live_configured(); out["live_supported_engine"]="SPOT_TRIANGULAR"; out["live_cooldown_ms"]=LIVE_COOLDOWN_MS
    ...
    out["diagnostic_model"]={"triangular_cost_bps":pricing.cost_bps(3,FEE_BPS,SLIP_BPS,0),"basis_cost_bps":pricing.cost_bps(2,FEE_BPS,SLIP_BPS,FUNDING_BUFFER_BPS),"futures_triangular_cost_bps":pricing.cost_bps(3,FEE_BPS,SLIP_BPS,FUNDING_BUFFER_BPS),"fee_bps":FEE_BPS,"slippage_bps":SLIP_BPS,"funding_buffer_bps":FUNDING_BUFFER_BPS,"min_net_bps":core.MIN_NET,"max_net_bps":core.MAX_NET,"stale_ms":core.STALE,"spread_already_in_gross":True}
    out["live_note"]="Canonical executable bid/ask model. Spread crossing is included in gross; slippage is an additional depth/impact allowance. Oldest quote timestamp governs leg freshness. Negative/insufficient-net candidates are rejected."
    return out
