...
LIVE_TRADING is false unless explicitly enabled in Render. Futures/basis and
futures-triangle signals remain paper-only in this release.
"""
import os, time, threading, logging
import arbitrage_engine_v6 as v6
from live_order_layer import enabled, execute_spot_triangle, LiveOrderError

log = logging.getLogger("cryptoalpha-v7")
LIVE_COOLDOWN_MS = max(1000, int(os.getenv("LIVE_ORDER_COOLDOWN_MS", "5000")))
_last_live_ms = 0.0

# Requested live configuration defaults.
        ...
        try:
            ...
            if enabled() and s and now - _last_live_ms >= LIVE_COOLDOWN_MS:
                ...
                if best.get('net_bps', 0) >= v6.MIN_NET:
                    _last_live_ms = now
                    try:
                        live_result = execute_spot_triangle(best, v6.spot)
                ...
                # Preserve v6 paper capture when live execution is disabled.
                if not enabled():
                    with v6.lock:
...
# Reuse v6 HTTP handler but expose v7 stats by replacing the stats function.
v6.stats = stats_v7
v6.scan = live_scan
