...
C=Config(); S=requests.Session()
if C.key:S.headers.update({'X-MBX-APIKEY':C.key})
...
def signed(method,path,params=None):
    if not C.key or not C.secret: raise RuntimeError('API credentials are required for signed requests')
...
def closes(limit=120): return [float(x[4]) for x in public('GET','/fapi/v1/klines',{'symbol':C.symbol,'interval':C.interval,'limit':limit})]
...
def atr_from_closes(xs,n=14): return mean(abs(xs[i]-xs[i-1]) for i in range(len(xs)-n,len(xs)))

def signal(xs):
    e9,e21=ema(xs,9),ema(xs,21); rr=rsi(xs); a=atr_from_closes(xs); last=xs[-1]
    if a<=0:return None
...
def exchange_rules():
    ...
    f={x['filterType']:x for x in s['filters']}; return float(f['LOT_SIZE']['stepSize']),float(f['LOT_SIZE']['minQty'])

def round_qty(q,step):
    d=Decimal(str(q)); st=Decimal(str(step)); return float((d/st).to_integral_value(rounding=ROUND_DOWN)*st)

def account_equity(): return float(next(x for x in signed('GET','/fapi/v2/balance') if x['asset']=='USDT')['balance'])
def position(): return float(signed('GET','/fapi/v2/positionRisk',{'symbol':C.symbol})[0]['positionAmt'])

def place(side,qty,stop_price,take_price):
    if C.dry:return {'dry_run':True}
...
def run_once():
    ...
    risk_cash=eq*C.risk; stop_distance=max(atr_from_closes(xs)*1.5,price*0.001)
    step,minq=exchange_rules(); qty=round_qty(risk_cash/stop_distance,step)
    if qty<minq: log.warning('Calculated qty %.8f below exchange minimum %.8f',qty,minq); return None
    stop=price-stop_distance if sig=='BUY' else price+stop_distance; take=price+stop_distance*1.5 if sig=='BUY' else price-stop_distance*1.5
    if not C.dry and position()!=0:return None
    result=place(sig,qty,round(stop,4),round(take,4)); notify(f'Binance Scalper V1: {sig} {C.symbol} qty={qty} price={price:.4f} mode={"DRY" if C.dry else "LIVE"}'); return result
