"""Dry-run-first Binance USD-M Futures meme scalper with new-listing sniper mode."""
import json
import logging
import math
import os
...
# New-listing sniper: newly discovered perpetuals receive a separate setup.
...
MEME_SYMBOLS = {"DOGE", "SHIB", "1000SHIB", "PEPE", "1000PEPE", "FLOKI", "BONK", "WIF",
...
metrics = {"events": 0, "signals": 0, "entries": 0, "exits": 0, "reconnects": 0,
           ...
           "score_reject": 0, "cost_reject": 0, "flow_reject": 0, "long_candidates": 0,
           "short_candidates": 0, "ai_boosts": 0, "cost_checks": 0, "new_listings": 0,
           "new_listing_entries": 0, "listing_refreshes": 0}
...
def exchange_symbols():
    ...
    return {x["symbol"].lower() for x in info["symbols"]
            if x.get("status") == "TRADING" and x.get("contractType") == "PERPETUAL"
            and x.get("quoteAsset") == "USDT"}

def discover_new_listings(allowed):
    now = time.time()
    fresh = []
    for s in allowed:
...
def universe():
    allowed = exchange_symbols()
    discover_new_listings(allowed)
    ...
    for t in tickers:
        ...
        try:
            ...
            if s in first_seen and now - first_seen[s] <= NEW_LISTING_WINDOW and qv >= NEW_LISTING_MIN_QV:
...
def score_symbol(s):
    ...
    if spread_bps > MAX_SPREAD_BPS * (1.5 if is_new else 1.0):
        metrics["spread_reject"] += 1; return None
    if len(ps) < 8 or len(fs) < 10:
        metrics["short_state"] += 1; return None
    ...
    recent, previous = list(fs[-5:]), list(fs[-10:-5])
    ...
    if vol < 0.00002 and not is_new:
        metrics["vol_reject"] += 1; return None
    long_score = (_clamp(imbalance) * 0.30 + _clamp(micro_edge / 0.0004) * 0.18 +
                  _clamp(momentum / 0.0010) * 0.22 + _clamp(flow_edge / 1.5) * 0.18 +
                  _clamp(directional_accel) * 0.12)
    short_score = (_clamp(-imbalance) * 0.30 + _clamp(-micro_edge / 0.0004) * 0.18 +
                   _clamp(-momentum / 0.0010) * 0.22 + _clamp(-flow_edge / 1.5) * 0.18 +
                   _clamp(-directional_accel) * 0.12)
    side = "BUY" if long_score > short_score else "SELL"
    signed_flow = flow_edge if side == "BUY" else -flow_edge
    if signed_flow < -0.15:
        metrics["flow_reject"] += 1; return None
    score = max(long_score, short_score)
    if len(ps) >= 15:
        ai = polymorph_decide(s.upper(), list(ps)[-40:])
        if ai.action == side:
            score = min(1.0, score + (0.08 if ai.confidence >= 2/3 else 0.03)); metrics["ai_boosts"] += 1
    ...
    if expected_edge <= round_trip_cost * (MIN_EDGE_MULT - 1.0):
        metrics["cost_reject"] += 1; return None
    if score < threshold:
        metrics["score_reject"] += 1; metrics["long_candidates" if long_score >= short_score else "short_candidates"] += 1; return None
    if is_new:
    ...
    return side, score, st["ask"] if side == "BUY" else st["bid"], target, stop, expected_edge, is_new

def enter(s, side, score, price, target, stop, expected_edge, is_new=False):
    now = time.time()
    with lock:
        if s in positions or len(positions) >= MAX_POSITIONS: return
        if is_new and sum(1 for p in positions.values() if p.get("new_listing")) >= NEW_LISTING_MAX_POSITIONS: return
        cooldown = NEW_LISTING_COOLDOWN if is_new else COOLDOWN
        if now - last_entry.get(s, 0.0) < cooldown: return
        positions[s] = {"side": side, "entry": price, "opened": now, "score": score, "tp": target, "sl": stop,
                        "expected_edge": expected_edge, "new_listing": is_new}
        last_entry[s] = now; metrics["entries"] += 1
        if is_new: metrics["new_listing_entries"] += 1
    log.warning("ENTRY %s %s score=%.3f edge=%.3f%% tp=%.3f%% sl=%.3f%% price=%.10g%s [DRY RUN]",
...
def manage_positions():
    ...
    with lock:
        for s, p in list(positions.items()):
            ...
            if reason:
                positions.pop(s, None); metrics["exits"] += 1
                perf = performance[s]; perf["trades"] += 1; perf["return"] += ret; perf["wins" if ret > 0 else "losses"] += 1
                exits.append((s, p, px, ret, reason, now - p["opened"]))
    for s, p, px, ret, reason, held in exits:
        log.warning("EXIT %s %s return=%.3f%% held=%.2fs%s", reason, s.upper(), ret * 100, held,
                    " NEW-LISTING" if p.get("new_listing") else "")

def on_message(_, raw):
    try:
        msg = json.loads(raw); d = msg.get("data", msg); event = d.get("e"); s = d.get("s", "").lower()
        if not s or s not in state: return
        metrics["events"] += 1; st = state[s]
        if event == "bookTicker":
            st["bid"], st["ask"] = float(d["b"]), float(d["a"]); st["bq"], st["aq"] = float(d["B"]), float(d["A"]); st["last_event"] = time.time()
        elif event == "aggTrade":
            px, qty = float(d["p"]), float(d["q"]); signed = -px * qty if d.get("m") else px * qty
            st["last"] = px; st["prices"].append(px); st["flow"].append(signed); st["flow_abs"].append(abs(signed)); st["last_event"] = time.time()
        else: return
        sig = score_symbol(s)
...
def run_ws(url, shard):
    while True:
        ...
        except Exception: log.exception("WS shard %d failed", shard)
        metrics["reconnects"] += 1; time.sleep(1)

def monitor():
    while True: manage_positions(); time.sleep(0.01)

def listing_monitor():
    while True:
        try:
            allowed = exchange_symbols(); metrics["listing_refreshes"] += 1
            fresh = discover_new_listings(allowed)
            meme_fresh = [s for s in fresh if is_meme_symbol(s)]
            if meme_fresh:
                log.warning("NEW LISTINGS QUEUED | %s", ",".join(s.upper() for s in meme_fresh))
                # Dynamically add newly listed meme contracts to the live stream state.
                for s in meme_fresh:
                    state.setdefault(s, make_state())
        except Exception: log.exception("listing monitor error")
...
def main():
    ...
    symbols = universe()
    if not symbols: raise RuntimeError("No Binance meme USDT perpetuals matched the configured liquidity filter.")
    for s in symbols: state[s] = make_state()
    shards = [[] for _ in range(min(WS_SHARDS, max(1, len(symbols))))]
    for i, s in enumerate(symbols): shards[i % len(shards)].append(s)
    log.warning("ENGINE STARTED | MEME-ONLY symbols=%d shards=%d dry_run=%s new_listing_window=%ss", len(symbols), len(shards), DRY_RUN, NEW_LISTING_WINDOW)
    ...
    while True:
        time.sleep(10)
        log.info("metrics events=%d signals=%d entries=%d exits=%d reconnects=%d positions=%d new_listings=%d new_entries=%d listing_refreshes=%d rejects={book:%d state:%d spread:%d vol:%d flow:%d cost:%d score:%d} ai_boosts=%d",
                 metrics["events"], metrics["signals"], metrics["entries"], metrics["exits"], metrics["reconnects"], len(positions),
                 metrics["new_listings"], metrics["new_listing_entries"], metrics["listing_refreshes"], metrics["no_book"], metrics["short_state"],
                 metrics["spread_reject"], metrics["vol_reject"], metrics["flow_reject"], metrics["cost_reject"], metrics["score_reject"], metrics["ai_boosts"])
