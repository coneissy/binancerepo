...
Fixes the startup deadlock where ranking required locally-built 1m history while
WebSocket subscriptions required ranking first. REST klines now seed history
for the discovered universe before scoring; the WebSocket keeps those bars live.
Also logs per-filter rejection counts so candidates=0 is diagnosable.
"""
...
BAR_MS=60000; WARMUP=int(os.getenv("WARMUP_BARS","25"))
START_EQUITY=float(os.getenv("SIM_START_EQUITY","10000")); BASE_RISK=float(os.getenv("BASE_RISK_PCT","0.0025")); MAX_NOTIONAL_X=float(os.getenv("MAX_POSITION_NOTIONAL_X","0.35")); MAX_GROSS_X=float(os.getenv("MAX_GROSS_EXPOSURE_X","1.5")); MAX_SIDE_X=float(os.getenv("MAX_SAME_SIDE_EXPOSURE_X","0.9")); DD_THROTTLE=float(os.getenv("DD_THROTTLE_PCT","0.08")); DD_STOP=float(os.getenv("DD_STOP_PCT","0.15")); VOL_CAP=float(os.getenv("VOL_CAP_PCT","0.015")); SHOCK_Z=float(os.getenv("SHOCK_Z","4.5")); KELLY_FRACTION=float(os.getenv("KELLY_FRACTION","0.20"))
...
state={}; bars={}; history={}; positions={}; last_entry={}; ranked=[]; desired=[]; regime="UNKNOWN"; market_regime="UNKNOWN"; kill_until=0; peak=START_EQUITY
metrics={"events":0,"bars":0,"signals":0,"entries":0,"exits":0,"wins":0,"losses":0,"net_pnl":0.0,"gross_profit":0.0,"gross_loss":0.0,"shock_kills":0}
...
def seed_history(s):
    """Seed local 1m history from REST so scoring never waits 25 minutes."""
    try:
        k=public("/fapi/v1/klines",{"symbol":s.upper(),"interval":"1m","limit":max(WARMUP+8,40)})
        h=history.setdefault(s,deque(maxlen=100)); now=int(time.time()*1000)
        for x in k[:-1]:
            t=int(x[0]); h.append({"t":t,"o":float(x[1]),"h":float(x[2]),"l":float(x[3]),"c":float(x[4]),"qv":float(x[7]),"buy":0.0,"sell":0.0,"n":int(x[8])})
        if k:
            x=k[-1]; p=float(x[4]); z=st(s); z["last"]=p
            bucket=(int(x[0])//BAR_MS)*BAR_MS
            bars[s]={"t":bucket,"o":float(x[1]),"h":float(x[2]),"l":float(x[3]),"c":p,"qv":float(x[7]),"buy":0.0,"sell":0.0,"n":int(x[8])}
            if z["bid"]<=0:z["bid"]=p
            if z["ask"]<=0:z["ask"]=p
        return True
    except Exception as e:
        log.debug("seed %s failed: %s",s,e); return False
...
def feat(s):
    ...
    atr=mean_safe([(x["h"]-x["l"])/max(x["c"],1e-12) for x in base],0.0); rv=q[-1]/max(mean_safe(q[-n:-1],1e-12),1e-12); ret=last/prev-1; ret5=last/c[-6]-1 if len(c)>=6 else ret; prevret=prev/c[-3]-1 if len(c)>=3 else 0; accel=ret-prevret; hi=max(x["h"] for x in base[-8:]); lo=min(x["l"] for x in base[-8:]); flow=(b["buy"]-b["sell"])/max(b["buy"]+b["sell"],1e-12); mid=(z["bid"]+z["ask"])/2; spread=(z["ask"]-z["bid"])/max(mid,1e-12)*10000; book=(z["bq"]-z["aq"])/max(z["bq"]+z["aq"],1e-12); ranges=[abs((x["c"]/h[-i-2]["c"])-1) for i,x in enumerate(h[-min(12,len(h)-1):])] if len(h)>3 else [atr]; vol=mean_safe(ranges,atr); zmove=abs(ret)/max(vol,1e-6)
    return {"c":last,"ret":ret,"ret5":ret5,"accel":accel,"atr":atr,"rv":rv,"flow":flow,"spread":spread,"book":book,"qv":b["qv"],"breakup":last>hi,"breakdn":last<lo,"z":zmove}
...
def higher(s):
    ...
    for iv,lim in (("1m",40),("5m",30)):
        ...
        c=[float(x[4]) for x in k]; v=[float(x[5]) for x in k]; out[iv]={"e9":ema(c,9),"e21":ema(c,21),"mom":c[-1]/c[-4]-1,"vr":v[-1]/max(mean_safe(v[-20:-1],1e-12),1e-12)}
    return out
...
def score_candidate(s,radar,age,q,rejects):
    f=feat(s)
    if not f:
        rejects["warmup_or_book"]+=1; return None
    if f["qv"]<MIN_1M_QV:
        rejects["1m_volume"]+=1; return None
    if f["spread"]>MAX_SPREAD_BPS:
        rejects["spread"]+=1; return None
    h=higher(s)
    if not h:
        rejects["higher_tf"]+=1; return None
    side="BUY" if f["ret"]>0 else "SELL"; signed=lambda x:x if side=="BUY" else -x
    impulse=min(max((f["rv"]-1)/2,0),1); accel=min(abs(f["accel"])/.004,1); flow=max(0,min((signed(f["flow"])+1)/2,1)); book=max(0,min((signed(f["book"])+1)/2,1)); z=min(f["z"]/3,1); breakv=1 if (f["breakup"] if side=="BUY" else f["breakdn"]) else 0; trend=1 if (h["1m"]["e9"]>h["1m"]["e21"])==(side=="BUY") else 0; ht=1 if (h["5m"]["e9"]>h["5m"]["e21"])==(side=="BUY") else 0
    exhaustion=max(0,z-.9)*(1-flow)*.8 + (0.5 if ((f["ret"]>0 and f["breakup"] and signed(f["flow"])<.05) or (f["ret"]<0 and f["breakdn"] and signed(f["flow"])<.05)) else 0)
    score=.28*impulse+.20*accel+.18*flow+.12*book+.10*z+.07*breakv+.05*(.55*trend+.45*ht)-exhaustion
    if regime=="CHOP":score-=.10
    if regime=="PANIC":score-=.50
    if regime=="TREND_UP" and side=="BUY":score+=.08
    if regime=="TREND_DOWN" and side=="SELL":score+=.08
    score=max(0,min(1,score)); expected=max(abs(f["ret"])*10000*1.8,f["atr"]*10000*2.0,abs(f["ret5"])*10000*1.2); edge=expected-(FEE_BPS+SLIPPAGE_BPS+f["spread"]); eligible=score>=ENTRY_SCORE and edge>=MIN_EDGE_BPS and impulse>=.25 and flow>=.56 and book>=.51 and (trend or ht) and regime!="PANIC"
    if not eligible:
        if score<ENTRY_SCORE:rejects["score"]+=1
        elif edge<MIN_EDGE_BPS:rejects["edge"]+=1
        elif impulse<.25:rejects["impulse"]+=1
        elif flow<.56:rejects["flow"]+=1
        elif book<.51:rejects["book"]+=1
        elif not (trend or ht):rejects["trend_alignment"]+=1
        elif regime=="PANIC":rejects["panic"]+=1
...
def enter(c):
    ...
    with lock:
        ...
        if px<=0:return
        positions[s]={**c,"entry":px,"notional":n,"opened":now,"mfe":0}; last_entry[s]=now; metrics["entries"]+=1; metrics["signals"]+=1
...
def manage():
    ...
    for s,p in list(positions.items()):
        z=st(s); px=z["bid"] if p["side"]=="BUY" else z["ask"]
        if px<=0:continue
        ret=(px/p["entry"]-1)*(1 if p["side"]=="BUY" else -1); p["mfe"]=max(p["mfe"],ret); age=time.time()-p["opened"]; f=feat(s); risk=max(p["atr"]*1.8,.0025); target=risk*2.4
        if f and f["z"]>SHOCK_Z:shock=True
        flip=f and ((f["flow"]<-.10) if p["side"]=="BUY" else (f["flow"]>.10)); decay=f and ((f["ret"]<0) if p["side"]=="BUY" else (f["ret"]>0)); reason=None
...
def rank():
    global ranked,desired,regime,market_regime
    regime=get_regime(); market_regime=regime; discovered=discover(); rejects={"warmup_or_book":0,"1m_volume":0,"spread":0,"higher_tf":0,"score":0,"edge":0,"impulse":0,"flow":0,"book":0,"trend_alignment":0,"panic":0}; cand=[]
    # Bootstrap local state before scoring and before WS subscriptions.
    for s,_,_,_ in discovered:
        if s not in history or len(history.get(s,()))<WARMUP:seed_history(s)
    for s,r,a,q in discovered:
        try:
            c=score_candidate(s,r,a,q,rejects)
            if c:cand.append(c)
        except Exception as e:rejects["exception"]=rejects.get("exception",0)+1; log.debug("candidate %s: %s",s,e)
    ...
    log.info("ALPHA 1M | regime=%s | discovered=%d | candidates=%d | eligible=%d | reject=%s | %s",regime,len(discovered),len(cand),sum(1 for x in cand if x["eligible"]),rejects," ".join(f'{x["symbol"]}:{x["score"]:.2f}/{x["edge_bps"]:.0f}bp/{x["side"]}' for x in ranked[:10]))
...
def ws_loop():
    while True:
        try:
            ...
            ws=websocket.WebSocketApp(WS_BASE+"?streams="+"/".join(streams),on_message=on_message,on_error=lambda w,e:log.debug("ws error %s",e),on_close=lambda w,c,m:log.info("ws reconnect")); ws.run_forever(ping_interval=20,ping_timeout=10); time.sleep(1)
...
def main():
    ...
    log.warning("1M ALPHA ENGINE START | discovery=%d execution=%d positions=%d | DRY_RUN=%s | REST_WARMUP=%d",DISCOVERY_N,EXECUTION_N,MAX_POSITIONS,DRY_RUN,WARMUP)
