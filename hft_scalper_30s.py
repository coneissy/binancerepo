...
MAX_POSITIONS=int(os.getenv("MAX_SIMULTANEOUS_POSITIONS","3")); MAX_SPREAD_BPS=float(os.getenv("MAX_SPREAD_BPS","10"))
...
def synthetic_features(sym):
    ...
    # Current bar plus completed history; current bar is allowed for live trigger.
    hist=list(getattr(s,"_dummy",[])) if False else []
    key=f"hist_{sym}"
    ...
    pos=(last-lo)/rng; ret1=last/prev-1; ret3=last/closes[-4]-1; accel=ret1-(prev/closes[-3]-1)
    ...
    breakout_up=last>hi; breakout_dn=last<lo
    return {"close":last,"ret1":ret1,"ret3":ret3,"accel":accel,"atr":atr,"rv":rv,"pos":pos,"ema9":e9,"ema21":e21,"rsi":rr,"flow":flow,"trades":trades,"spread":spread,"qv":b["qv"],"up":breakout_up,"dn":breakout_dn}
...
def universe():
    ...
    return sorted(out,key=lambda x:(x[2],x[1]),reverse=True)[:60]
...
def rank():
    ...
    for sym,q,radar in universe():
        try:
            ...
            expected=max(f["atr"]*10000*1.8,abs(f["ret3"])*10000*1.4,abs(f["accel"])*10000*2)
            edge=expected-cost
    ...
    log.info("30S TOP-%d | regime=%s | %s",TOP_N,market_regime," ".join(f'{x["symbol"]}:{x["score"]:.2f}/{x["edge_bps"]:.0f}bp' for x in ranked[:8]))

def confirm(c):
    if c["score"]<ENTRY_SCORE or c["edge_bps"]<MIN_EDGE_BPS or c["regime"]=="PANIC":return False
    if c["rv"]<1.35 or c["flow"]<.18 or abs(c["accel"])<.00025 or c["spread"]>MAX_SPREAD_BPS:return False
    return True
...
def manage():
    ...
    for s,p in list(positions.items()):
        q=st(s); px=q["bid"] if p["side"]=="BUY" else q["ask"]; ret=(px/p["entry"]-1)*(1 if p["side"]=="BUY" else -1); age=now-p["opened"]
        p["mfe"]=max(p["mfe"],ret); p["mae"]=min(p["mae"],ret); risk=max(p["atr"]*1.6,.0020); target=risk*2.8
        f=synthetic_features(s); flow_flip=f and ((f["flow"]<-.10) if p["side"]=="BUY" else (f["flow"]>.10)); momentum_flip=f and ((f["ret1"]<0) if p["side"]=="BUY" else (f["ret1"]>0))
        reason=None
...
def on_message(_,raw):
    try:
        d=json.loads(raw); d=d.get("data",d); ev=d.get("e")
        if ev=="bookTicker":
            s=d["s"].lower(); q=st(s); q["bid"]=float(d["b"]); q["ask"]=float(d["a"]); q["bq"]=float(d["B"]); q["aq"]=float(d["A"])
        elif ev=="aggTrade":
            add_trade(d["s"].lower(),float(d["p"]),float(d["q"]),bool(d["m"]),int(d["T"]))
    except Exception as e:log.debug("ws: %s",e)

def ws_loop():
    while True:
        syms=list(dict.fromkeys(desired_symbols));
        if not syms:time.sleep(2);continue
        streams=[f"{s}@aggTrade" for s in syms]+[f"{s}@bookTicker" for s in syms]
        url=WS_BASE+"?streams="+"/".join(streams)
        try:
            ws=websocket.WebSocketApp(url,on_message=on_message); ws.run_forever(ping_interval=15,ping_timeout=8)
...
def main():
    ...
    log.warning("30-SECOND HIGH-RR ENGINE | SYNTHETIC BARS | DRY RUN ONLY")
    threading.Thread(target=ws_loop,daemon=True).start(); last_rank=0
    while True:
        now=time.time()
        if now-last_rank>=RANK_REFRESH:
            try:rank()
            except Exception as e:log.warning("rank cycle: %s",e)
            last_rank=now
        for c in ranked:
            if c["symbol"] in desired_symbols:enter(c)
