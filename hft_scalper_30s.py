...
BASE=os.getenv("BINANCE_BASE_URL","https://demo-fapi.binance.com"); WS_BASE=os.getenv("BINANCE_WS_BASE_URL","wss://fstream.binance.com/stream"); DRY_RUN=os.getenv("DRY_RUN","true").lower()=="true"
DISCOVERY_N=int(os.getenv("DISCOVERY_N","30")); EXECUTION_N=int(os.getenv("EXECUTION_N","10")); RANK_REFRESH=float(os.getenv("RANK_REFRESH_SECONDS","20")); MAX_POSITIONS=int(os.getenv("MAX_SIMULTANEOUS_POSITIONS","10"))
MIN_24H_QV=float(os.getenv("MIN_24H_QUOTE_VOLUME","500000")); MIN_1M_QV=float(os.getenv("MIN_1M_QUOTE_VOLUME","15000")); MAX_SPREAD_BPS=float(os.getenv("MAX_SPREAD_BPS","10")); ENTRY_SCORE=float(os.getenv("ENTRY_SCORE","0.72")); MIN_EDGE_BPS=float(os.getenv("MIN_EDGE_BPS","12"))
FEE_BPS=float(os.getenv("EST_FEE_BPS","4")); SLIPPAGE_BPS=float(os.getenv("EST_SLIPPAGE_BPS","3")); COOLDOWN=float(os.getenv("ENTRY_COOLDOWN_SECONDS","60")); MAX_HOLD=float(os.getenv("MAX_HOLD_SECONDS","300")); BAR_MS=60000; WARMUP=int(os.getenv("WARMUP_BARS","25"))
...
logging.basicConfig(level=os.getenv("LOG_LEVEL","INFO"),format="%(asctime)s %(levelname)s %(message)s"); log=logging.getLogger("alpha-1m"); http=requests.Session(); lock=threading.RLock()
...
def public(path,params=None):
    for i in range(3):
        try:r=http.get(BASE+path,params=params,timeout=8); r.raise_for_status(); return r.json()
...
def score_candidate(s,radar,age,q):
    ...
    side="BUY" if f["ret"]>0 else "SELL"; signed=lambda x:x if side=="BUY" else -x; impulse=min(max((f["rv"]-1)/2,0),1); accel=min(abs(f["accel"])/.004,1); flow=max(0,min((signed(f["flow"])+1)/2,1)); book=max(0,min((signed(f["book"])+1)/2,1)); z=min(f["z"]/3,1); breakv=1 if (f["breakup"] if side=="BUY" else f["breakdn"]) else 0; trend=1 if (h["1m"]["e9"]>h["1m"]["e21"])==(side=="BUY") else 0; ht=1 if (h["5m"]["e9"]>h["5m"]["e21"])==(side=="BUY") else 0
...
def rank():
    ...
    regime=get_regime(); market_regime=regime; cand=[]
...
def main():
    ...
    log.warning("1M ALPHA ENGINE START | discovery=%d execution=%d positions=%d | DRY_RUN=%s",DISCOVERY_N,EXECUTION_N,MAX_POSITIONS,DRY_RUN); threading.Thread(target=ws_loop,daemon=True).start(); last=0
    while True:
        now=time.time()
