...
LIVE_SPOT = os.getenv("LIVE_SPOT_TRADING", "true").lower() in {"1", "true", "yes", "on"}
...
def enabled(kind=None):
    if not LIVE or not API_KEY or not API_SECRET or _halted:
        return False
...
def status():
    return {"halted": _halted, "session_loss_usdt": round(_loss, 8), "risk_budget_usdt": round(RISK_BUDGET, 8), "remaining_risk_usdt": round(max(0, RISK_BUDGET-_loss), 8), "spot_enabled": enabled("spot"), "futures_enabled": enabled("futures")}
...
def _signed(base, method, path, params):
    params = dict(params); params.setdefault("timestamp", int(time.time()*1000)); params.setdefault("recvWindow", RECV_WINDOW)
...
def _public(base, path, params=None):
    q=urllib.parse.urlencode(params or {}, doseq=True); req=urllib.request.Request(base+path+("?"+q if q else ""), headers={"User-Agent":"cryptoalpha-live-v2/1"})
    try:
        with urllib.request.urlopen(req, timeout=5) as r: return json.loads(r.read().decode())
    except Exception as e: raise LiveOrderError(str(e)) from e

def spot_account():
    if not enabled("spot"): raise LiveOrderError("live spot disabled or halted")
...
def fut_account():
    if not enabled("futures"): raise LiveOrderError("live futures disabled or halted")
...
def _min_notional(filters):
    f=filters.get("NOTIONAL", filters.get("MIN_NOTIONAL",{})); return float(f.get("notional", f.get("minNotional", "0")))
...
def _risk_notional(balance):
    return min(MAX_NOTIONAL, max(0.0, RISK_BUDGET-_loss), float(balance))
...
def _filled_qty(order):
    q=order.get("executedQty",order.get("origQty",0)); return float(q or 0)

def _guard(symbols, info, notional):
    for s in symbols:
        _,f=_symbol_info(info,s); lot=_qty_filter(f); step=float(lot.get("stepSize","0.000001"));
        if _min_notional(f)>0 and notional < _min_notional(f): return False, f"{s} minimum {_min_notional(f)} exceeds hard cap {RISK_BUDGET:.8f}"
    return True,""

def execute_spot_triangle(opportunity, price_store):
    global _last_order
    if not enabled("spot"): return {"executed":False,"reason":"spot live disabled"}
    ...
    a,b=parts[1],parts[2]; syms=[a+"USDT",a+b,b+"USDT"]; info=spot_info(syms); acct=spot_account(); balance=_balance_spot(acct,"USDT"); notional=_risk_notional(balance)
    ok,reason=_guard(syms,info,notional)
    if not ok:return {"executed":False,"reason":reason,"risk_budget_usdt":RISK_BUDGET}
    _,f=_symbol_info(info,syms[0]); step=float(_qty_filter(f).get("stepSize","0.000001")); qty=_round_down(notional/float(price_store[syms[0]][1]),step)
    if qty<=0:return {"executed":False,"reason":"quantity below lot size"}
    _last_order=now; orders=[]
    try:
        o1=spot_order(syms[0],"BUY",qty); orders.append(o1); aq=_filled_qty(o1)
        o2=spot_order(syms[1],"SELL",aq); orders.append(o2); bq=_filled_qty(o2)
        o3=spot_order(syms[2],"SELL",bq); orders.append(o3)
        return {"executed":True,"engine":"SPOT_TRIANGULAR","orders":orders,"notional_usdt":notional}
    except Exception:
        trip("incomplete spot triangle; manual reconciliation required"); raise

def execute_futures_triangle(opportunity, price_store):
    global _last_order
    if not enabled("futures"): return {"executed":False,"reason":"futures live disabled"}
    if float(opportunity.get("net_bps",0))<MIN_NET_BPS:return {"executed":False,"reason":"below threshold"}
    now=int(time.time()*1000)
    if now-_last_order<COOLDOWN_MS:return {"executed":False,"reason":"cooldown"}
    parts=opportunity.get("path","").split("->")
    if len(parts)!=4 or parts[0]!="USDT" or parts[-1]!="USDT":return {"executed":False,"reason":"invalid futures triangle"}
    a,b=parts[1],parts[2]; syms=[a+"USDT",a+b,b+"USDT"]; info=fut_info(); acct=fut_account(); balance=_balance_fut(acct); notional=_risk_notional(balance); ok,reason=_guard(syms,info,notional)
    if not ok:return {"executed":False,"reason":reason,"risk_budget_usdt":RISK_BUDGET}
    _,f=_symbol_info(info,syms[0]); step=float(_qty_filter(f).get("stepSize","0.001")); qty=_round_down(notional/float(price_store[syms[0]][1]),step)
    if qty<=0:return {"executed":False,"reason":"quantity below lot size"}
    _last_order=now; orders=[]
    try:
        o1=fut_order(syms[0],"BUY",qty); orders.append(o1); aq=_filled_qty(o1)
        o2=fut_order(syms[1],"SELL",aq); orders.append(o2); bq=_filled_qty(o2)
        o3=fut_order(syms[2],"SELL",bq); orders.append(o3)
        return {"executed":True,"engine":"FUTURES_TRIANGULAR","orders":orders,"notional_usdt":notional}
    except Exception:
        trip("incomplete futures triangle; manual reconciliation required"); raise

def execute_basis(opportunity, spot_prices, fut_prices):
    global _last_order
    if not enabled("futures") or not LIVE_SPOT:return {"executed":False,"reason":"spot/futures live disabled"}
    if float(opportunity.get("net_bps",0))<MIN_NET_BPS:return {"executed":False,"reason":"below threshold"}
    now=int(time.time()*1000)
    if now-_last_order<COOLDOWN_MS:return {"executed":False,"reason":"cooldown"}
    s=opportunity.get("symbol",""); side=opportunity.get("direction","");
    if not s or s not in spot_prices or s not in fut_prices:return {"executed":False,"reason":"stale prices"}
    info=fut_info(); acct=fut_account(); notional=_risk_notional(_balance_fut(acct)); ok,reason=_guard([s],info,notional)
    if not ok:return {"executed":False,"reason":reason,"risk_budget_usdt":RISK_BUDGET}
    _,f=_symbol_info(info,s); step=float(_qty_filter(f).get("stepSize","0.001")); qty=_round_down(notional/fut_prices[s][1],step)
    if qty<=0:return {"executed":False,"reason":"quantity below futures lot size"}
    _last_order=now
    try:
        # Hedge mode is intentionally not assumed. One-way mode is used so each leg opens an offsetting position.
        if side=="BUY_SPOT_SELL_FUTURES":
            so=spot_order(s,"BUY",_round_down(notional/spot_prices[s][1],step)); fo=fut_order(s,"SELL",qty)
        else:
            # Spot shorting is not available on ordinary Binance Spot; reject rather than mis-execute.
            return {"executed":False,"reason":"SELL_SPOT_BUY_FUTURES requires spot margin/short support and is disabled in this safe executor"}
        return {"executed":True,"engine":"SPOT_FUTURES_BASIS","spot_order":so,"futures_order":fo,"notional_usdt":notional}
    except Exception:
        trip("incomplete spot-futures hedge; manual reconciliation required"); raise
