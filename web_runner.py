...
# Cryptoalpha 3M: 15M context -> 5M ICT/SMC -> 3M execution.
...
def _api_failover(path, params=None):
    ...
    last = None
    for base in [current] + [b for b in _BASES if b != current]:
        try:
            engine.BASE = base
    ...
    raise last or RuntimeError("No Binance Futures REST endpoint available")
...
def _fresh_seed(symbol, interval, limit=100, force=False):
    now = time.time(); key = (symbol, interval)
    with _FRAME_LOCK:
        due = force or interval == "3m" or now - _FRAME_AT.get(key, 0.0) >= _FRAME_REFRESH[interval]
        if not due and interval in engine.hist.get(symbol, {}): return True
    try:
        ok = _ORIGINAL_SEED(symbol, interval, limit)
        if ok:
            with _FRAME_LOCK: _FRAME_AT[key] = time.time()
        return ok
...
def _ensure_frames(symbol):
    return all(_fresh_seed(symbol, i, 100) for i in ("3m", "5m", "15m"))
...
def _optimized_rank():
    ...
    cand = []
    for s, radar, age, q in d[:inspect_n]:
        try:
    ...
    try:
        ...
        else: engine.regime = "UNKNOWN"
    except Exception: engine.regime = "UNKNOWN"
    engine.log.info("CRYPTOALPHA 3M | regime=%s | discovered=%d | inspected=%d | ranked=%d | eligible=%d | reject=%s | %s", engine.regime, len(d), len(d[:inspect_n]), len(engine.ranked), sum(x.get("eligible",False) for x in cand), reject, " ".join(f'{x["symbol"]}:{x["score"]:.2f}/{x["side"]}/{x.get("key_level","-")}' for x in engine.ranked))
...
def get_stats():
    with engine.lock:
        ...
        equity = float(engine.equity); peak = float(engine.peak_equity); positions = list(engine.positions.values())
    closed = int(m.get('wins',0)) + int(m.get('losses',0))
    dd = max(0, (peak-equity)/peak) if peak > 0 else 0
    return {
        'status':'ok','service':'cryptoalpha-3m-engine','engine':'cryptoalpha-3m-ict-smc','dry_run':True,
        'uptime_seconds':round(time.time()-START_TIME,1),'regime':regime,'regime_source':'15M_CONTEXT_5M_TREND','entry_timeframe':'3M',
        'universe_mode':'MEME_TOP_GAINERS_TOP_MOVERS_NEW_LISTINGS','universe_ranked':len(ranked),'discovery_n':engine.DISCOVERY_N,'execution_n':engine.EXECUTION_N,
        'top_10':[x.get('symbol','').upper() for x in ranked[:10]],
        'top_10_detail':[{k:x.get(k) for k in ('symbol','side','score','edge','flow','eligible','new','ict','sweep','fvg','displacement','trend15','trend5','key_level','volatility','volatility_label','radar')} for x in ranked[:10]],
        'open_positions':len(positions),'entries':int(m.get('entries',0)),'exits':int(m.get('exits',0)),'wins':int(m.get('wins',0)),'losses':int(m.get('losses',0)),
        'win_rate_pct':round((m.get('wins',0)/closed)*100,2) if closed else 0,'net_pnl_pct':round(float(m.get('pnl',0))/max(engine.START,1e-9)*100,4),
        'equity':round(equity,2),'peak_equity':round(peak,2),'drawdown_pct':round(dd*100,4),'trading_halted':bool(engine.trading_halted),
        'risk_pct':engine.RISK*100,'sl_pct':engine.SL_PCT*100,'tp_pct':engine.TP_PCT*100,'signals':int(m.get('signals',0)),
...
def main():
    port=int(os.getenv('PORT','10000')); threading.Thread(target=engine.main,name='cryptoalpha-3m',daemon=True).start(); server=ThreadingHTTPServer(('0.0.0.0',port),Handler); print('Cryptoalpha 3M dashboard on :%d'%port,flush=True); server.serve_forever()
