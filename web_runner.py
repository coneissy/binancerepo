...
def get_stats():
    ...
    return {
        'status': 'ok', 'service': 'binance-1m-engine', 'engine': 'alpha-htf-1m', 'dry_run': True,
        ...
        'top_10': [x.get('symbol', '').upper() for x in ranked[:10]], 'open_positions': open_positions,
...
DASHBOARD = '''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Binance HTF Alpha Engine</title></head><body><h1>Binance HTF Alpha Engine</h1><pre id="x">Loading...</pre><script>async function u(){try{let r=await fetch('/stats.json?x='+Date.now(),{cache:'no-store'});x.textContent=JSON.stringify(await r.json(),null,2)}catch(e){x.textContent='OFFLINE'}}u();setInterval(u,3000)</script></body></html>'''

class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        path = self.path.split('?', 1)[0]
        if path in ('/', '/health', '/healthz'):
            self._json({'status':'ok','service':'binance-1m-engine','engine':'alpha-htf-1m','dry_run':True,'regime_source':'1H_PRIMARY_15M_CONFIRM','universe_mode':'ALL_USDT_PERPETUAL_LIVE_VOLUME'})
...
def main():
    port=int(os.getenv('PORT','10000')); worker=threading.Thread(target=engine.main,name='alpha-1m',daemon=True); worker.start(); server=ThreadingHTTPServer(('0.0.0.0',port),HealthHandler); print('Health/stats server for HTF alpha engine listening on :%d'%port,flush=True); server.serve_forever()
