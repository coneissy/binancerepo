...
def get_stats():
    ...
    return {
        'status': 'ok', 'service': 'binance-1m-engine', 'engine': 'alpha-htf-1m-adaptive', 'dry_run': True,
        ...
        'top_10_detail': [{k:x.get(k) for k in ('symbol','side','score','ensemble_score','strategy','edge','flow','z','rv','eligible','new')} for x in ranked[:10]],
...
DASHBOARD = '''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ALPHA 1M Adaptive Engine</title><style>body{font-family:system-ui;margin:20px;max-width:1100px}pre{white-space:pre-wrap;word-break:break-word}</style></head><body><h1>ALPHA 1M Adaptive Engine</h1><p>Dynamic universe · regime filter · strategy ensemble · adaptive weights · trailing exits · paper mode</p><pre id="x">Loading...</pre><script>async function u(){try{let r=await fetch('/stats.json?x='+Date.now(),{cache:'no-store'});x.textContent=JSON.stringify(await r.json(),null,2)}catch(e){x.textContent='OFFLINE'}}u();setInterval(u,3000)</script></body></html>'''

class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        path = self.path.split('?', 1)[0]
        if path in ('/', '/health', '/healthz'):
            self._json({'status':'ok','service':'binance-1m-engine','engine':'alpha-htf-1m-adaptive','dry_run':True,'regime_source':'1H_PRIMARY_15M_CONFIRM','universe_mode':'ALL_USDT_PERPETUAL_LIVE_VOLUME'})
...
def main():
    port=int(os.getenv('PORT','10000')); worker=threading.Thread(target=engine.main,name='alpha-1m',daemon=True); worker.start(); server=ThreadingHTTPServer(('0.0.0.0',port),HealthHandler); print('Health/stats server for adaptive ALPHA 1M engine listening on :%d'%port,flush=True); server.serve_forever()
